...
--- then we allow setting exactly one such variable to $\infty$ ($-\infty$), in order
to achieve an infinite objective function value.
Then if a new constraint enters at a later step
in the algorithm that this infinite-value variable (or any other variable) violates,
we use the constraint to solve a smaller LP recursively as usual.

If we introduce an infinite-value variable and then reach the end of the algorithm
without violating any constraints, then we conclude that the LP is unbounded,
and output an unbounded solution involving exactly one infinite-valued variable.
If this is the output to the entire problem then we conclude finally that the LP
is unbounded. Otherwise, if it is the solution to a sub-problem, then we continue
adding constraints to this infinite-valued solution as above.
