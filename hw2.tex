...
more than $m$ edges.
\end{proof}
After $\sqrt{m}$ phases of Dinitz, the shortest augmenting path has length
...
= n + \frac{1}{2} d(t) > n$$ which yields desirable contradiction. 
\end{proof}
After $2n^{2/3}$ iterations of Dinitz, the shortest path has length $d(t)\geq 2n^{2/3}$. 
There exist two consecutive $D_j$ and $D_{j+1}$ that both have less than $n^{1/3}$ vertices.
Therefore, the number of edges cross from $D_j$ to $D_{j+1}$ is less than $n^{2/3}$.
Hence the algorithm can run at most another $n^{2/3}$ iterations. The total runtime is $O(n^{2/3})$.
\section*{Problem 3}
